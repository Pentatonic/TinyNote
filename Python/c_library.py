...
# ------------------------------------------------------------------------------
'''
get global variable in library
value
'''
from ctypes import *

# get global int
g_count = c_int.in_dll(g_libabc, "g_count").value

# get global pointer to array of structure
class struct_alpha(Structure):
    _fields_ = [
        ("aa", c_long),
        ("bb", c_char),
        ("cc", c_char*128),
...
ptrAlpha = POINTER(struct_alpha)
g_slotlist = ptrAlpha.in_dll(g_libabc, "g_list")
